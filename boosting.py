...
if len(sys.argv) >= 2:
    ...
    for i in range(1, 30):
        ...
        booster = AdaBoostClassifier(dtree)
    ...
    final_booster = AdaBoostClassifier(final_dtree, n_estimators=300,
                                       random_state=np.random.RandomState())
    final_booster.fit(X_train, y_train)
    y_pred = final_booster.predict(X_test)
    ...
    plot_curves(final_dtree, title, X, y, train_sizes=np.linspace(.1, 1.0, 9))
